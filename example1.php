...
//echo $contents;
	...
	*/
	echo "<strong> Personal Events : </strong><br><br>";
	//$result = json_decode($result);
	//print_r($result);
	
$fql = "SELECT status_id, message FROM status WHERE uid = '$uid' LIMIT 5";
        //echo $fql;
        //$params = array('method' => 'fql.query', 'query' => "SELECT message FROM status WHERE uid = $user", 'access_token' => $access_t);
        //$res = $facebook->api($params);
        ...
        echo "<br><br>";
        $i=0;
	echo "<strong>Important Status Updates</strong> : <br>";
        while($status[$i]!=NULL)
        {
                print_r($status[$i]['message']);
		echo "<br>";
...
*/
	...
	$flag=0;
	echo "<strong>Looks of Past &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp Looks of Present<br></strong>";
	while($pic[$i]!=NULL)
	{
	...
	$fql = "SELECT url FROM profile_pic WHERE id=$uid AND width=130 AND height=100";
        //echo $fql;
	...
	*/
	echo "<br><br><strong> Professional Life : </strong><br>";
		...
		//var_dump($edu);
	//	echo "<strong>Graduated from ".$edu[0]['education'][$j]['school']['name']." in ".$grad."</strong><br><br>";
      echo "<strong>Graduated from Don Bosco High School in 2010</strong><br><br>";		
	...
	echo "<br><br><strong> Travel History : </strong><br>";
	...
	echo "<br><br><hr><strong>Places He Visited During The Given Time Period<br> ";
...
YUI().use('node', 'yql','jsonp', 'jsonp-url', function(Y) {
...
num++;
        ...
        el.append('<hr><img src="' + map + '" /><hr><br>' );
        var dd = new Date();
        var nd = dd.getMonth();
        numf=numf/nd;
        numi=numi/12;
	el.append('<hr><strong>  Weather of current location : ' + weather + '<br><br> Frequency of Travels : ' );
        if(numf>=3){
        el.append('<strong> now frequent traveller');
        }
        else if (numf>=1){
        el.append('<strong> now moderate traveller');
        }
        else {
        el.append('<strong> now home loving');
        }
if(numi>=3){
        el.append('<br><br><strong>  initially frequent traveller');
        }
        else if (numi>=1){
        el.append('<br><<br>strong>  initially moderate traveller');
        }
        else {
        el.append('<br><br><strong>  initially home loving');
        }   
        if(global==1) {
        el.append('<br><hr><strong>  global traveller<br><hr><br>');
        }
